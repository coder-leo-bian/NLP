...
# 花萼长度，花萼宽度，花瓣长度，花瓣宽度 sepal_len, sepal_width, petal_len, petal_width
def deal_iris():
    data = load_iris().data
    data1 = data[:, :2]
    ...
    Mulinomial_model(datas, target)


def Mulinomial_model(datas, target):
    models = []
    for d in datas:
...
def drawing(models):
    ...
    # mp.tight_layout()
    mp.savefig(config.APP_IMAGES_TXT + 'GaussianNB.png')
    mp.show()

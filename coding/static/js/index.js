...
//	提交按钮
	$("#submit_btn").click(function(e){		
		...
		if($('input').eq(1).val() != $('input').eq(2).val()){
		    $('input').eq(2).parent().next(".tips").text("两次密码输入不一致请重新输入");
		    $('input').eq(2).parent().next(".tips").css("color",'red');
		    e.preventDefault();
		    return;
			}
		if($('input').eq(1).val().length<6){
		    $('input').eq(1).parent().next(".tips").text("密码个数不能少于6位");
		...
		$('input').eq(4).blur(function(){
		if($(this).val().toUpperCase()!=$("#code").text().toUpperCase()){
			$(this).parent().next().next("div").text("验证码不正确");
			$(this).parent().next().next("div").css("color",'red');
		    e.preventDefault();
		    return;
